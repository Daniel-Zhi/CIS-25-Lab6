# include "Player.h"
    ...
    Player::Player(int capacity, string name, int health, int attackPower, int x , int y) : inventory(capacity), name(name), health(health), maxHealth(health), attackPower(attackPower), x(x), y(y) 
    ...
    Player::int getX() const{
        return x;
    }

    Player::int getY() const{
        return y;
    }

    Player::void setX(int x){
        this->x = x;
    }

    Player::void setY(int y){
        this->y = y;
    }
